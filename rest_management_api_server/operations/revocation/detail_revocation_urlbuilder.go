...
func (o *DetailRevocationURL) Build() (*url.URL, error) {
	...
	var _path = "/revocations/{id}"

	id := o.ID
	if id != "" {
		_path = strings.ReplaceAll(_path, "{id}", id)
	} else {
		return nil, errors.New("id is required on DetailRevocationURL")
	}
...
func (o *DetailRevocationURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DetailRevocationURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DetailRevocationURL")
	}
